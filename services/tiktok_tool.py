...
# Import Config để lấy đường dẫn chuẩn
try:
    from config import Config
except ImportError:
    # Fallback nếu chạy file này độc lập để test
    sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from config import Config

# --- 1. SETUP ĐƯỜNG DẪN & IMPORT SCRAPER ---
# Thêm đường dẫn cha vào sys.path để tìm thấy folder TT_Content_Scraper
...
def _get_slideshow_images(video_id):
    """
    Tìm tất cả ảnh slideshow của video từ folder scraper.
    Pattern: tiktok_picture_{video_id}_{index}.jpeg
    Returns: List các đường dẫn ảnh, đã sorted theo index
    """
    content_dir = os.path.join(Config.SCRAPER_DIR, "content_files")
    if not os.path.exists(content_dir):
        return []
    
    images = []
    pattern = f"tiktok_picture_{video_id}_"
    
    for filename in os.listdir(content_dir):
        if filename.startswith(pattern) and filename.endswith(".jpeg"):
            full_path = os.path.join(content_dir, filename)
            # Extract index từ filename để sort
            try:
                index = int(filename.replace(pattern, "").replace(".jpeg", ""))
                images.append((index, full_path, filename))
            except:
                images.append((999, full_path, filename))
    
    # Sort theo index
    images.sort(key=lambda x: x[0])
    return [(path, fname) for _, path, fname in images]
...
def parse_metadata(raw_data, video_id, tiktok_url):
    ...
    if 'video_metadata' in raw_data:
        ...
        clean_meta = {
            ...
            "is_slideshow": file_meta.get('is_slide', False),
...
def download_tiktok_full_data(tiktok_url):
    ...
    # B4: Phân loại nội dung (Video hay Slideshow?)
    is_slideshow = False
    if os.path.exists(raw_video_path):
        print("   🎥 Phát hiện: VIDEO (.mp4)")
    elif os.path.exists(raw_audio_path):
        print("   🖼️ Phát hiện: SLIDESHOW (.mp3 + ảnh)")
        is_slideshow = True
    else:
        print(f"❌ Lỗi: Không tìm thấy file tải về. (ID: {video_id})")
        return None
    ...
    try:
        if is_slideshow:
            # SLIDESHOW: Chỉ có file MP3, không có MP4
            # 1. Copy MP3 gốc ra
            shutil.copy(raw_audio_path, final_music_path)
            
            # 2. Hack: Copy MP3 thành file đuôi .mp4
            # Lý do: Để các hàm xử lý video sau này (upload drive, AI) không bị lỗi file path
            shutil.copy(raw_audio_path, final_video_path) 
        else:
            # VIDEO THƯỜNG
            # 1. Copy Video MP4
    ...
    try:
        ...
        clean_metadata['is_slideshow'] = is_slideshow
        
        # Thêm danh sách ảnh cho slideshow
        slideshow_images = []
        slideshow_images_abs = []  # Đường dẫn tuyệt đối cho OCR
        if is_slideshow:
            images = _get_slideshow_images(video_id)
            # Lưu URL path cho frontend (serve từ /scraper_images)
            slideshow_images = [f"/scraper_images/{fname}" for _, fname in images]
            # Lưu đường dẫn tuyệt đối cho OCR processor
            slideshow_images_abs = [path for path, _ in images]
            clean_metadata['slideshow_images'] = slideshow_images
            clean_metadata['slideshow_images_abs'] = slideshow_images_abs  # NEW: cho OCR
            print(f"   🖼️ Tìm thấy {len(slideshow_images)} ảnh slideshow")
        ...
        return {
            "local_path": final_video_path,  # Đường dẫn video (hoặc mp3 giả video)
            "music_path": final_music_path,
            "meta_path": final_meta_path,
            "filename": f"{video_id}.mp4",
            "metadata": clean_metadata,
            "slideshow_images": slideshow_images,  # URL paths cho frontend
            "slideshow_images_abs": slideshow_images_abs  # NEW: Đường dẫn tuyệt đối cho OCR
        }
